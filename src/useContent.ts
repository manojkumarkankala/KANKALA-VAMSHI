...
export function useContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const siteContent: SiteContent = {
          ...
          ...(sc?.about_video_url ? { about_video_url: sc.about_video_url } : {}),
      ...(sc?.hero_video_url ? { hero_video_url: sc.hero_video_url } : {}),
        };
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'education' }, () => load())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'qualities' }, () => load())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'stats' }, () => load())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'portfolio_items' }, () => load())
      .subscribe();
